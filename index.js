...
let token = {};
try { token = JSON.parse(fs.readFileSync('./data/token.json', 'UTF-8')); }
catch {
...
async function GetBearerToken(){
    try {
        ...
        const index = response.data.indexOf('gc:authToken');
...
async function ClaimDailyRewards(){
    try {
        const response = await axios.post('https://missions-api.gamersclub.com.br/player/daily-rewards/claim', {}, claim_options);
